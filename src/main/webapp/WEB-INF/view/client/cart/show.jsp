...
<c:forEach items="#{items}" var="item">
  ...
  <c:if test="${item.productVariant.quantity > 0}">
    <form:form action="/cart/updateDown/${item.id}" method="post" modelAttribute="items">
  ...
  <c:if test="${item.quantity < item.productVariant.quantity}">
    <form:form action="/cart/updateUp/${item.id}" method="post" modelAttribute="items">
      <button type="submit" class="btn btn-outline-secondary">+</button>
    </form:form>
  </c:if>
  </div>
    ...
    <td style="display: flex; justify-content: center; align-items: center; height: 100px;">
      <form:form action="/cart/delete-item-in-cart/${item.id}" method="post" modelAttribute="items">
        <button type="submit" class="button-delete-item">
